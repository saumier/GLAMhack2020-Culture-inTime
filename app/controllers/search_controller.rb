class SearchController < ApplicationController
  ...
  def index
    if params[:start]
      ...
      if params[:query]
        @productions = Production.where(date_of_first_performance: date_range).order(:date_of_first_performance).where('lower(label) LIKE :search  OR lower(locality) LIKE :search OR lower(location_label) LIKE :search OR lower(country) LIKE :search', search: "%#{params[:query].downcase}%")
      else
